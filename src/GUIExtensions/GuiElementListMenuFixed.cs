...
namespace TeaLib
{
	namespace GuiExtensions
	{
		...
		public class GuiElementListMenuFixed : GuiElementTextBase
		{
			...
			public GuiElementListMenuFixed(ICoreClientAPI capi, string[] values, string[] names, int selectedIndex, SelectionChangedDelegate onSelectionChanged, ElementBounds bounds, CairoFont font, bool multiSelect) : base(capi, "", font, bounds)
			{
			...
			public void ComposeDynamicElements()
			{
				...
				//visibleBounds = Bounds.FlatCopy().WithSizing(Bounds.horizontalSizing, ElementSizing.Fixed).WithAlignment(EnumDialogArea.FixedTop); // AngelFix
				visibleBounds = ElementBounds
				.Percentual(EnumDialogArea.FixedTop, 1, 0)
				.WithSizing(Bounds.horizontalSizing, ElementSizing.Fixed)
				.WithParent(Bounds.ParentBounds); // AngelFix
				...
				// AngelFix - replace hardcoded scrollbar width with existing `scrollbarWidth` variable
				...
				scrollbar.Bounds.WithFixedSize(scrollbarWidth, visibleBounds.fixedHeight - 3).WithFixedPosition(expandedBoxWidth / RuntimeEnv.GUIScale - scrollbarWidth, 0).WithFixedPadding(0, 2);
			...
			public override void RenderInteractiveElements(float deltaTime)
			{
				if (expanded)
				{
					...
					api.Render.Render2DTexturePremultipliedAlpha(
						...
						// AngelFix - visibleBounds.renderX and .renderY doesn't work with Percentual bounds, replaced with Bounds.renderX/Y as used for hoverTexture
						(int)Bounds.renderX,
						(int)(Bounds.renderY + Bounds.InnerHeight),
